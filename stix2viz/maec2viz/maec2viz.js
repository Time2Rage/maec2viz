...
let embeddedRelationships = new Map([
    ...
    [null, null],
    ["behavior", [
        ["action_refs", "refers-to", true]
    ]],
    ["collection", [
        ["entity_refs", "observed together", true],
        ["observable_refs", "observed together", true]
    ]],
    ["malware-action", [
        ["input_object_refs", "uses", false],
        ["output_object_refs", "uses", true]
    ]],
    ["malware-family", [
        ["common_code_refs","uses artifact",true],
        ["common_behavior_refs", "behaves like", true]
    ]],
    ["malware-instance",[
        ["instance_object_refs", "is",true]
    ]],
    // Added subtypes
    ["analysis_metadata",[
        ["tool_refs","uses",false]
    ]],
    ["capability", [
        ["behavior_refs","does",false],
        ["parent_ref","belongs to", false]
    ]],
    ["field_data", [
        ["parent_ref","of",true]
    ]],
    ...
    ["development_environment",[
        ["parent_ref","used",false],
        ["tool_refs","uses",true],
        ["debugging_file_refs","describes",false]
    ]],
    ["binary_obfuscation",[
        ["parent_ref","applied",false]
    ]],
    ["dynamic_features",[
        ["parent_ref", "of", false],
        ["behavior_refs","",true],
        ["action_refs","",true],
        ["network_traffic_refs"," ",true],
...
/**
 * @author Rene Dralle - B00138163
 * The dissectMaec() function splits maec objects into more finegrained entries
 * based on the maec-types encountered in the parsed MAEC TLOs. After creating Map
 * objects of the encountered subtypes they are deleted from the original object.
 * Each subtype introduces the "parent_ref" key to ensure consistency in association.
 * @param maecObjects A map of maec-objects
 * @returns dissectedMaec Map of the input objects split into detail
 */
function dissectMaec(maecObjects){
    let dissectedMaec_array = []
    for (let maec_object of maecObjects){
        // Static Features
        if (maec_object.has("static_features")) {
            let static_features = maec_object.get("static_features")
            static_features.set("id","static_features--"+ maec_object.get("id"))
            static_features.set("type","static_features")
            static_features.set("parent_ref",maec_object.get("id"))
            // Development environment
            if (static_features.has("development_environment")){
                let devEnv = static_features.get("development_environment")
                devEnv.set("id","development-environment--"+ static_features.get("id"))
                devEnv.set("type","development_environment")
                devEnv.set("parent_ref",static_features.get("id"))
                dissectedMaec_array.push(devEnv)
                static_features.delete("development_environment")
            }
            // Binary Obfuscation
        ...
        maec_object.delete("static_features")
        }            
        // Dynamic Features
        if (maec_object.has("dynamic_features")) {
            let dynamic_features = maec_object.get("dynamic_features")
            dynamic_features.set("id","dynamic_features--"+ maec_object.get("id"))
            dynamic_features.set("type","dynamic_features")
            dynamic_features.set("parent_ref",maec_object.get("id"))
            dissectedMaec_array.push(dynamic_features)
        maec_object.delete("dynamic_features")
        }
        // Capabilities
        ...
        // Field Data
        if (maec_object.has("field_data")) {
            let field_data = maec_object.get("field_data")
            field_data.set("type","field_data")
            field_data.set("id","field_data--" + maec_object.get("id"))
            field_data.set("parent_ref",maec_object.get("id"))
            dissectedMaec_array.push(field_data)
        maec_object.delete("field_data")
        }        
        // API Call
        if (maec_object.has("api_call")) {
            let api_call = maec_object.get("api_call")
            api_call.set("type","api_call")
            api_call.set("id","api_call--" + maec_object.get("id"))
            api_call.set("parent_ref",maec_object.get("id"))
            dissectedMaec_array.push(api_call)
        maec_object.delete("api_call")
        } 
        // Push cleaned Maec Object
        dissectedMaec_array.push(maec_object)
    }
    return dissectedMaec_array
}
...
function nameForStixObject(stixObject, stixIdToName, nameCounts, config=null)
{
    ...
    if (!name)
    {
        ...
        /**
         * @author Rene Dralle - B00138163
         * Extension of the naming scheme. This snippet was added to cover the dynamic
         * creation of names in the MAEC report by automatted tooling.
         * This creation results in the schema "name" : {"value":"abcd","confidence":80}
         * Therefore this snippet determines if the "name" is a Map object and parses the
         * value as the name.
         */
            if (stixObject.has("name"))
                baseName = stixObject.get("name");
            if (stixObject.has("function_name"))
                baseName = stixObject.get("function_name");
            if (stixObject.has("method"))
                baseName = stixObject.get("method");
            if (baseName instanceof Map)
                baseName = baseName.get("value")
        // end of addition
        if (!baseName)
...
function stixTypeToIconURL(stixType, iconPath, iconFileName)
{
    ...
    if (!iconFileName)
        iconFileName = stixType.replaceAll("-", "_")
            + ".png";
...
/**
 * @author Rene Dralle - B00138163
 * Specialised normalization of Observable Objects entries in the MAEC report format.
 * This function serves as an extension for the *normalizeContent* function below.
 * After the integrity of the MAEC report is established, this function expects a Map object
 * with the observable objects.
 * This function is necessary, since the observable objects don't carry their own uuid.
 * @param obsEntries Map of observable objects found in the report
 * @return An array of well-formed observable object entries
 */
function normalizeObservables(obsObjects)
{
    let observable_objects_array = []
    for (let id of obsObjects.keys()){
        let observable_object = new Map(obsObjects.get(id))
        observable_object.set("id",id);
        observable_objects_array.push(observable_object)
    }
    return observable_objects_array
}
...
*/
    {
        if (stixContent.get("type") === "package" 
        && parseInt(stixContent.get("schema_version")) >= 5){
            ...
            observable_objects = normalizeObservables(observable_objects)
            // Append all observable objects to the stixObjects Map Array
            for (const obsObject of observable_objects){
                stixObjects.push(obsObject)
            }
            // Check for relationship section and populate
            if (stixContent.get("relationships")){
                let relationships = stixContent.get("relationships")
                for (const rel of relationships){
                    stixObjects.push(rel)
                }
            }
            else
